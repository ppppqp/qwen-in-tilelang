...
class TinyKvCache(ABC):
    ...
    ) -> tuple[torch.Tensor, torch.Tensor, int, torch.Tensor | str | None]:
        """
...
class BatchingKvCache(TinyKvCache):
    ...
    def update_and_fetch(
        ...
        keys = torch.zeros(
            (self.max_active_requests, H, seq_len, D),
            dtype=keys.dtype,
            device=keys.device,
        )
        values = torch.zeros(
        ...
        masks = torch.full(
            (self.max_active_requests, mask_length, seq_len),
            -torch.inf,
            dtype=keys.dtype,
            device=keys.device,
        )
        for b in range(B):
            ...
            if mask is None or (isinstance(mask, str) and mask == "causal"):
                masks[b, :, seq_len - S : seq_len] = causal_mask(
                    mask_length, S, dtype=key.dtype, device=key.device
                )
    ...
    def update_and_fetch_paged(
        ...
        return PagedKvMetadata(
            ...
            block_table=torch.tensor(rows, dtype=torch.int32, device=keys.device),
            context_lens=torch.tensor(
                context_lens, dtype=torch.int32, device=keys.device
            ),
