...
def rope(X, offset, BLOCK_N, BLOCK_S, BLOCK_H, BLOCK_D):
    ...
    with T.Kernel(
        ...
        O_local = T.alloc_fragment((BLOCK_N, BLOCK_S, BLOCK_H, BLOCK_D), dtype)
        cos_basis = T.alloc_fragment((S, BLOCK_D), dtype)
        sin_basis = T.alloc_fragment((S, BLOCK_D), dtype)
        for i, j in T.Parallel(BLOCK_H, BLOCK_D):
            global_row_idx = BLOCK_H * pid_h + i
            global_col_idx = BLOCK_D * pid_d + j + offset
            freq = T.pow(10000.0, -global_col_idx / half_D)
            cos_basis[i, j] = T.cos(global_row_idx * freq)
            sin_basis[i, j] = T.sin(global_row_idx * freq)
